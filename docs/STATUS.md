...
> Снимок: **2026-04-25**, после Day 6 hardening + post-Day-6 cleanup (Person A).
> Тесты: **332 passed / 0 failed** (`test_health.py::test_unknown_endpoint_returns_404` падает только под Python 3.14 — Django/template-context bug, не наш код, в Docker не повторяется).
...
- `dd3bf0d` Day 6 Person A: SoftTimeLimit handlers во всех 4 artifact workers + 4 pipeline tasks + packager → терминальное состояние вместо зависания
...
- (uncommitted) Post-Day-6 cleanup (Person A): docker-compose `-Q` фикс, `.gitignore`, regenerate cleanup старых mp4, `completed_at` на FAILED jobs, +4 теста (24/24 в hardening-сюите)
...
- `cc9512d` Post-Day-6 frontend fixup (Person A→Person B territory): `USE_REAL_API` под Vitest mock-режим, URL submission error handling, обновление stale-тестов
...
### Решено в post-Day-6 (uncommitted)
- [x] **`docker-compose.yml` — celery worker без `-Q`** → теперь `-Q default,video,text_artifacts,graphics` (без этого задачи на video/text_artifacts/graphics висят на брокере)
- [x] **`.gitignore`** → добавлены `.claude/`, `.coverage`, `htmlcov/`
- [x] **`frontend/src/pages/LandingPage.jsx` — реальные POST'ы** → сделано в Day 5 Person B (`0010bb5`)

### Зависит от пользователя
- [ ] **`.env` локальный** — не создан. Скопировать `.env.example` → `.env`, вписать `OPENAI_API_KEY` + `ANTHROPIC_API_KEY`
- [ ] **ffmpeg на хосте** (опционально, только если запускаешь без Docker на Windows)
- [ ] **Один реальный подкаст** для интеграционного теста full pipeline upload→ZIP
...
### Day 6 — Polish (Person B) — done (`138ba16`)
- [x] Edge cases SPEC §10.6: SSE reconnect через polling fallback, failed artifacts UI, retry button через `regenerateArtifact`
- [x] Mobile-responsive (sm: брейкпоинты на Hero/Header/cards)
- [x] Accessibility: `aria-label`/`role`/`aria-busy`/семантические `<main>`/`<section>`/`<article>` везде
- [x] Visual polish: skeleton loaders в `ArtifactCard` (QUEUED/PROCESSING), toast-нотификации через pub/sub (`Toaster.jsx`)
...
### Сделано в post-Day-6 cleanup
- ✅ **Регенерация video clip cleanup** — старый `clip_<idx>_v<N>.mp4` удаляется с диска после успешной записи `_v<N+1>.mp4` (best-effort, ошибка → warning в логи)
- ✅ **`completed_at` на FAILED** — теперь стампится в `_fail_job` и во всех 4 FAILED-путях `package_job`. UI/аналитика может вычислить "time-to-failure"
...
- **Двойной bump version при regenerate**: view (`api/views/jobs.py:regenerate_artifact`) делает `version+=1`, потом worker (`generate_video_clip` с `regenerate=True`) делает ещё `+1`. Реальная версия в DB — N+2 после одного regenerate. Не критично (UI всё равно показывает свежее значение через poll/SSE), но v(N+1) "теряется". Можно убрать bump в view — но это изменение API контракта, отложим до post-MVP.

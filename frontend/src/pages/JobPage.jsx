...
export default function JobPage() {
  ...
  if (job.status === 'FAILED') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-4 text-center">
        <p className="text-red-600 font-semibold text-lg">Processing failed</p>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Top bar */}
      <div className="bg-white border-b border-gray-100 sticky top-0 z-10 px-4 py-3 shadow-sm">
        <div className="max-w-5xl mx-auto flex items-start justify-between gap-4 flex-wrap">
          <div className="min-w-0">
            {job.analysis?.episode_title ? (
              ...
              <h1 className="text-lg font-bold text-gray-400">Processing your episode…</h1>
            )}
            {job.analysis?.hook && (
              <p className="text-xs text-gray-500 mt-0.5 truncate">{job.analysis.hook}</p>
            )}
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-8">
        ...
        {/* Empty state — no artifacts created yet (pre-fan-out) */}
